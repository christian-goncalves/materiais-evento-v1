...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (saldoGlobal < quantidade) {
      return res.status(400).json({ error: 'Estoque global insuficiente para transferencia' });
    }

    const movimentoId = await nextEstoqueIdFromSheet();
    const createdAt = nowIso();
    const origem = body.origem ? String(body.origem) : 'transferencia_manual';
    ...
    return res.status(200).json({
      ...
      movimento: {
        ...
        tipo: 'transferencia_companheiro',
      },
    });
  } catch (err) {
